...
def _split_lines(text: str, max_len: int = 92):
    """Split a long string into roughly max_len chunks without breaking words."""
    lines, buf = [], ""
...
def build_pdf(project: str,
    ...
    """
    Returns PDF bytes. Kept simple/robust for Streamlit Cloud.
    serp_rows: list of dicts (keyword, position, title, link, our_site)
    history_rows: list of dicts with keys at least ('timestamp','lvi')
    """
    # ----- canvas setup
    buf = BytesIO()
    ...
    if logo_bytes:
        try:
            ...
            # top-right corner
            c.drawImage(img, 16.0*cm, h-3.2*cm, width=3.0*cm, preserveAspectRatio=True, mask='auto')
        except Exception:
            pass
    ...
    if serp_rows:
        ...
        for r in serp_rows[:8]:
            ...
            ttl = r.get("title","")
            rows.append([
                (kw[:20]+"…") if len(kw)>20 else kw,
                str(r.get("position","")),
                (ttl[:60]+"…") if len(ttl)>60 else ttl
            ])
    ...
    # ----- second page: Recommendations & FAQ
    c.showPage()
    c.setFont("Helvetica-Bold", 16)
    ...
    for line in _split_lines(plan.get("suggested_title",""), 110):
        c.setFont("Helvetica", 10); c.drawString(2*cm, y, line); y -= 0.35*cm
    y -= 0.25*cm
    ...
    if faqs_md:
        for raw in faqs_md.splitlines():
            ...
            for L in _split_lines(line, 110):
                c.setFont("Helvetica", 10); c.drawString(2*cm, y, L); y -= 0.32*cm
                if y < 3*cm:
                    c.showPage(); y = h-2.0*cm
    else:
        c.setFont("Helvetica-Oblique", 10)
        c.drawString(2*cm, y, "(No FAQs generated)")
        y -= 0.5*cm

    # JSON-LD
    y -= 0.4*cm
    c.setFont("Helvetica-Bold", 12); c.drawString(2*cm, y, "FAQPage JSON-LD")
    y -= 0.45*cm
    jsonld = plan.get("faq_jsonld","").strip()
    if jsonld:
        for line in _split_lines(jsonld, 100):
            c.setFont("Courier", 9); c.drawString(2*cm, y, line); y -= 0.3*cm
            if y < 2.5*cm:
                c.showPage(); y = h-2.0*cm
    else:
        c.setFont("Helvetica-Oblique", 10); c.drawString(2*cm, y, "(No JSON-LD generated)")
        y -= 0.5*cm
